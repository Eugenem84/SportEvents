...
* `messages.pin` разрешён **владельцу** беседы; сообществу, которое в чужой беседе лишь администратор, VK отвечает `925`. Бот всё равно пробует закрепить анонс (это работает в беседах сообщества), а отказ пишет в лог и продолжает работать.
