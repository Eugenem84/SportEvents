...
* Записаться / в очередь / мои записи / отмена — готово (полная игра → резерв, отмена с подъёмом первого из резерва и уведомлением в беседе)
* Создание игры админом и анонс «Записываемся» — готово (список записавшихся живёт в одном сообщении, обновляется через `messages.edit`; при заполнении кнопка меняется на «В резерв»)
* Расписание чата (сверх плана) — готово (`chat_game_slots`, экран «настройки» с кнопками, день и время сообщением «вс 10:00»; «создать игру» без даты берёт ближайший слот, есть защита от дубля анонса)
* Слэш-команды — готово (`/games`, `/my`, `/cancel`, `/create`, `/settings`, `/slot`, `/help`); подсказки при вводе «/» — ручная настройка сообщества, API-метода у VK нет
